...
function renderQuestion() {
  ...
  if (els.metaCounter) els.metaCounter.textContent = `${pos}/${total}`;
  if (els.progressBar) els.progressBar.style.width = `${Math.round((pos / total) * 100)}%`;
  ...
  if (els.btnPrev) els.btnPrev.disabled = session.index === 0;
  if (els.btnNext) els.btnNext.disabled = !answered;
...
function renderBank(filterText = "") {
  ...
  const qlist = allQuestions.slice();
  ...
  filtered.forEach((q, i) => {
    ...
    q.choices.forEach((c, idx) => {
      ...
      li.textContent = c;
      if (idx === q.answerIndex) li.style.fontWeight = "700";
      ul.appendChild(li);
    });
...
function wireEvents() {
  if (els.btnStart) {
    els.btnStart.addEventListener("click", () => {
      const count = clampInt(els.inputCount?.value ?? 10, 1, 9999);
      const mode = els.selectMode?.value ?? "instant"; // "instant" | "exam"
  ...
  if (els.btnPrev) els.btnPrev.addEventListener("click", goPrev);
  ...
  if (els.btnRetryWrong) {
    els.btnRetryWrong.addEventListener("click", () => {
      ...
      if (els.checkOnlyWrong) els.checkOnlyWrong.checked = true;
      show(els.home);
    });
  }
  ...
  window.addEventListener("keydown", (e) => {
    if (!session) return;
    if (els.quiz && els.quiz.classList.contains("hidden")) return;

    if (e.key === "ArrowRight" && els.btnNext && !els.btnNext.disabled) goNext();
    if (e.key === "ArrowLeft" && els.btnPrev && !els.btnPrev.disabled) goPrev();
  });
}
